...
export class BinaryCrossEntropy {
  constructor() {}
  public static objective(actual: Vector2D, predicted: Vector2D): number {
  ...
  public static gradient(actual: Vector2D, predicted: Vector2D): Vector2D {
    // eq: (1/m) * ((1/(1+np.exp(- Z))) - Y)
    const m = actual.shape[1];

    return vectorTs.multiply(
      1 / m,
